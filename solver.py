...
class Solver(object):
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            out_src, out_cls_spks, _ = self.D(mc_real)
            d_loss_real = - torch.mean(out_src)
            d_loss_cls_spks = self.classification_loss(out_cls_spks, spk_label_org)

            # Compute loss with fake mc feats.
            newList = []
            for j in range(len(spk_label_trg)):
                newList.append(list[spk_label_trg[j]])
            newArray = np.array(newList)
            newArray = torch.from_numpy(newArray).cuda(1)

            mc_fake = self.G(mc_real, spk_c_trg)
            out_src, out_cls_spks, out_spk_vec = self.D(mc_fake.detach())
            d_loss_fake = torch.mean(out_src)
            d_loss_spk_vec = self.mse_loss(out_spk_vec, newArray)
            ...
            d_loss = d_loss_real + d_loss_fake + self.lambda_cls * d_loss_cls_spks + self.lambda_gp * d_loss_gp + d_loss_spk_vec
            self.reset_grad()
            d_loss.backward()
            ...
            loss['D/loss_spk_vec'] = d_loss_spk_vec.item()
            ...
            if (i+1) % self.n_critic == 0:
                ...
                mc_fake = self.G(mc_real, spk_c_trg)
                out_src, out_cls_spks, out_spk_vec = self.D(mc_fake)
                g_loss_fake = - torch.mean(out_src)
                g_loss_cls_spks = self.classification_loss(out_cls_spks, spk_label_trg)
                g_loss_spk_vec = self.mse_loss(out_spk_vec, newArray)
                ...
                # Backward and optimize.
                g_loss = g_loss_fake + self.lambda_rec * g_loss_rec + self.lambda_cls * g_loss_cls_spks + g_loss_spk_vec
                self.reset_grad()
                g_loss.backward()
                ...
                loss['G/loss_spk_vec'] = g_loss_spk_vec.item()

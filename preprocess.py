...
def get_spk_world_feats(spk_fold_path, mc_dir_train, mc_dir_test, sample_rate=16000):
    ...
    for wav_file in tqdm(train_paths):
        ...
        os.makedirs(join(mc_dir_train, wav_nam[:4]), exist_ok=True)
        ...
        np.save(join(mc_dir_train, wav_nam[:4], wav_nam.replace('.wav', '.npy')), normed_coded_sp, allow_pickle=False)
    
    for wav_file in tqdm(test_paths):
        wav_nam = basename(wav_file)
        os.makedirs(join(mc_dir_test, wav_nam[:4]), exist_ok=True)
        f0, timeaxis, sp, ap, coded_sp = world_encode_wav(wav_file, fs=sample_rate)
        normed_coded_sp = normalize_coded_sp(coded_sp, coded_sps_mean, coded_sps_std)
        np.save(join(mc_dir_test, wav_nam[:4], wav_nam.replace('.wav', '.npy')), normed_coded_sp, allow_pickle=False)
    return 0
...
if __name__ == '__main__':
    ...
    num_workers = argv.num_workers if argv.num_workers is not None else 60
